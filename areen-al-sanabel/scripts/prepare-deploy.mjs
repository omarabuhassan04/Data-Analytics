...
الخطوات المتبقّية قبل النشر:
  ...
  3) هيّئ قاعدة البيانات. ملفات الهجرة صارت بصياغة Postgres صحيحة،
     وأمر build ينفّذ migrate deploy تلقائيًا عند كل نشر — فلا يلزم
     تشغيله يدويًا إلا لقاعدة محلية. البذر يبقى يدويًا مرة واحدة:
       npx prisma db seed

     لا تستخدم db push: يتخطّى سجل الهجرات ويترك _prisma_migrations
     فارغًا، فتفشل عمليات migrate deploy اللاحقة.
